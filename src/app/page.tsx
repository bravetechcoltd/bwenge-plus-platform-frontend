      ...
      <div className="flex min-h-[calc(100vh-4rem)]">
        ...
        >
          <button
            ...
            className="absolute right-2 top-2 mb-3 w-6 h-6 bg-primary text-white rounded-full flex items-center justify-center shadow-lg hover:bg-blue-700 z-50"
          >
          ...
          {isSidebarOpen && (
            <div className="p-4">
              <div className="mb-4">
                <h3 className="text-sm font-bold text-gray-900 mb-1">Browse by Institution</h3>
                <p className="text-xs text-gray-600">Select to view course categories</p>
              </div>
              ...
              {isLoadingPublic ? (
                    ...
                    <div key={institution.id} className="border border-gray-200 rounded-lg overflow-hidden">
                      ...
                      >
                        ...
                        <ChevronDown
                          className={`w-4 h-4 text-gray-400 transition-transform ${
                            selectedInstitution === institution.id ? "rotate-180" : ""
                          }`}
                        />
                      </button>

                      {selectedInstitution === institution.id && institution.categories.length > 0 && (
                        <div className="px-3 pb-3 space-y-1 bg-gray-50">
                          {institution.categories.map((category) => (
                            <button
                              key={category.id}
                              onClick={() => handleCategoryClick(institution.slug, category.name)}
                              className="w-full text-left px-3 py-2 text-xs text-gray-700 hover:bg-blue-50 hover:text-blue-600 rounded flex items-center justify-between group"
                            >
                      ...
                      {selectedInstitution === institution.id && institution.categories.length === 0 && (
                        <div className="px-3 pb-3 bg-gray-50">
                          <p className="text-xs text-gray-500 py-2 text-center">No course categories available</p>
                        </div>
                      )}
                    </div>
                  ))}
                ...
                <div className="text-center py-8">
                  <p className="text-sm text-gray-500">No institutions available</p>
                </div>
              )}
            </div>
          )}
          ...
          {!isSidebarOpen && (
            <div className="hidden md:flex flex-col items-center py-4 gap-3 mt-5">
...
// Updated FeaturedCourses component with proper category filtering
                ...
                >
                  ...
                  <div className="relative h-48 overflow-hidden">
                    {course.thumbnail_url ? (
                      ...
                      <div className={`w-full h-full bg-gradient-to-br ${levelInfo.color} to-gray-600 flex items-center justify-center`}>
                    ...
                    <div className="absolute top-3 left-3">
                      <Badge className={course.course_type === "MOOC" ? "bg-blue-500 text-white" : "bg-purple-500 text-white"}>
                        {course.course_type === "MOOC" ? <Globe className="w-3 h-3 mr-1" /> : <Shield className="w-3 h-3 mr-1" />}
                        {course.course_type}
                      </Badge>
                    </div>
                    ...
                    <div className="absolute top-3 right-3">
                      <Badge className={`${levelInfo.color} text-white flex items-center gap-1`}>
                        {levelInfo.icon}
                  ...
                  <div className="p-6">
                    ...
                    <div className="flex items-center gap-2 mb-3">
                      ...
                      <span className="text-sm text-gray-600">
                        {course.instructor?.first_name} {course.instructor?.last_name || "Expert"}
                      </span>
                    </div>
  ...
  return (
    <GoogleOAuthProvider clientId={googleClientId || ''}>
      <div className="min-h-screen bg-white">
        ...
        />
        {/* Pass activeCategory to FeaturedCourses and force re-render when category changes */}
