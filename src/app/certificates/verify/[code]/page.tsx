    ...
    const verifyCertificate = async () => {
      try {
        ...
        const response = await fetch(
          `${process.env.NEXT_PUBLIC_API_URL}/certificates/verify/${code}`,
          {
        ...
        if (response.ok) {
          ...
          if (data.success && data.valid) {
            ...
            // Invalid certificate but API returned 200
            setError(data.message || "Certificate is not valid")
            setVerification(data)
          }
        } else {
          // Response is not OK (400, 404, 500, etc.)
        ...
        if (err.name === 'AbortError') {
          setError("Verification timeout. Please check your connection and try again.")
        } else {
          setError("Failed to verify certificate. Please check the verification code and try again.")
        }
