...
export default function InstitutionAdminDashboard() {
  ...
  const fetchDashboardData = async () => {
    ...
    try {
      ...
      // Debug: Log the API URL and configuration
      ...
      setDebugInfo(`Attempting to connect to: ${endpoint}`);
      ...
      if (error.name === 'TypeError' && error.message === 'Failed to fetch') {
        errorMessage = 'Unable to connect to the API server. Please check:\n' +
                      '1. Is the API server running?\n' +
                      '2. Is NEXT_PUBLIC_API_URL configured correctly?\n' +
                      '3. Is there a CORS issue?\n' +
                      `Current API URL: ${process.env.NEXT_PUBLIC_API_URL || 'NOT SET'}`;
