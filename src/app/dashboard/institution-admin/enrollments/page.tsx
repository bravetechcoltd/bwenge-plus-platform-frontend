// frontend/app/dashboard/institution-admin/enrollments/page.tsx
...
export default function InstitutionAdminEnrollmentsPage() {
  ...
  const toggleMessage = (requestId: string) => {
    setExpandedMessages((prev) => ({ ...prev, [requestId]: !prev[requestId] }))
  }
                        ...
                        <TableRow key={request.id}>
                          ...
                          <TableCell className="max-w-[220px]">
                            {request.request_message ? (
                              <div className="text-sm text-muted-foreground max-w-[220px] whitespace-normal break-words">
                                <p className={`mb-1 ${expandedMessages[request.id] ? "line-clamp-none" : "line-clamp-2"}`}>
                                  {expandedMessages[request.id] || request.request_message.length <= 10
                                    ? request.request_message
                                    : `${request.request_message.slice(0, 10)}...`}
                                </p>
                                {request.request_message.length > 10 && (
                                  <button
                                    type="button"
                                    onClick={() => toggleMessage(request.id)}
                                    className="text-xs font-medium text-primary hover:underline"
                                  >
                                    {expandedMessages[request.id] ? "Show less" : "Show more"}
                                  </button>
                                )}
                              </div>
                            ) : (
                      ...
                      {filteredApprovals.map((approval) => (
                        <TableRow key={approval.id}>
                          ...
                          <TableCell>
                            <div className="flex items-center gap-2">
                              <BookOpen className="w-4 h-4 text-muted-foreground" />
                              <span className="font-medium">{approval.course.title}</span>
                            </div>
                          </TableCell>
                          <TableCell>
