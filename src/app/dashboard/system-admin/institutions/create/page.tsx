...
export default function CreateEditInstitutionPage() {
  ...
  useEffect(() => {
    if (isEditMode && selectedInstitution) {
      setFormData({
        ...
        max_instructors: selectedInstitution.settings?.max_instructors ?? 10,
      });
  ...
  const validate = () => {
    ...
    if (formData.max_instructors < 1) {
      newErrors.max_instructors = "Maximum instructors must be at least 1";
    }
...
const handleSubmit = async (e: React.FormEvent) => {
  ...
  // ✅ FIXED: Changed to match Multer field configuration
  if (logoFile) {
    // The field name MUST match what's in your Multer config: "logoFile"
  ...
  try {
    if (isEditMode && institutionId) {
      ...
      const result = await dispatch(createInstitution(formDataToSend)).unwrap();
              ...
              <div className="space-y-4">
                ...
                <div>
                  ...
                  <input
                    ...
                    onChange={(e) =>
                      handleChange("max_instructors", parseInt(e.target.value))
                    }
                    className="w-full px-4 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                  />
                </div>
