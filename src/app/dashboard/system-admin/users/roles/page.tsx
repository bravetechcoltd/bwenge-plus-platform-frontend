    ...
    <div className="container mx-auto px-4 py-6 space-y-5">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-0.5">Platform roles and permission sets</p>
        </div>
          ...
          {roles.map(role => {
            ...
            return (
              <button key={role.id} onClick={() => openEdit(role)}
                className="bg-card rounded-xl border text-left p-4 hover:shadow-md transition-all group"
                ...
                <div className="flex items-center justify-between mb-2">
                  ...
                  {role.is_system_role && <Lock className="w-3 h-3 text-muted-foreground shrink-0" />}
                </div>
                <p className="text-2xl font-black text-foreground">{(role.user_count || 0).toLocaleString()}</p>
                ...
                <p className="text-[10px] text-muted-foreground mt-1">{pct(role.permissions)}% permissions</p>
              </button>
            );
          })}
        </div>
          ...
          <table className="w-full">
            ...
            <tbody>
              {loading ? (
                ...
                const c = color(role.name);
                return (
                  <tr key={role.id} className="border-b border-border hover:bg-muted/50/40 transition-colors">
                    <td className="px-5 py-3.5">
                      <div className="flex items-center gap-3">
                        ...
                        <div>
                          <p className="font-semibold text-sm text-foreground">{role.display_name}</p>
                          <p className="text-xs text-muted-foreground max-w-[220px] truncate">{role.description}</p>
                        </div>
                      </div>
                    </td>
                    <td className="px-5 py-3.5">
                    ...
                    <td className="px-5 py-3.5 text-right">
                      <div className="flex items-center justify-end gap-2">
                        <span className="text-sm text-muted-foreground">{role.permissions.length}/{ALL_KEYS.length}</span>
        ...
        <DialogContent className="!max-w-4xl w-[95vw] max-h-[90vh] overflow-y-auto">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mt-1">
            ...
            <div className="space-y-4">
              <div>
                <label className="text-xs font-semibold text-muted-foreground mb-1.5 block">Internal Key</label>
              ...
              <div className="rounded-xl bg-muted/50 border border-border p-4 space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-xs font-semibold text-muted-foreground">Permission Coverage</span>
                  <span className="text-xs font-bold text-muted-foreground">{form.permissions.length}/{ALL_KEYS.length}</span>
                </div>
                ...
                <div className="flex gap-2 pt-1">
                  ...
                  <button onClick={() => setForm(p => ({ ...p, permissions: [] }))}
                    className="text-xs font-semibold text-muted-foreground hover:underline">Clear all</button>
                </div>
              </div>

              {selectedRole && (
                <div className="flex items-center gap-2 text-sm text-muted-foreground rounded-xl border border-border px-4 py-3">
                  <Users className="w-4 h-4 text-muted-foreground" />
                  <span><strong className="text-foreground">{(selectedRole.user_count || 0).toLocaleString()}</strong> users assigned</span>
                </div>
              )}
            </div>
                ...
                {PERMISSION_GROUPS.map(group => {
                  ...
                  return (
                    <div key={group.group} className="border border-border rounded-xl overflow-hidden">
                      <div className="flex items-center justify-between px-4 py-2.5 bg-muted/50/60 cursor-pointer select-none"
                        ...
                        <div className="flex items-center gap-3" onClick={e => e.stopPropagation()}>
                          ...
                          {someOn && !allOn && (
                            <span className="text-[10px] text-primary font-bold bg-primary/10 px-1.5 py-0.5 rounded">partial</span>
                          )}
                        </div>
                        <div className="flex items-center gap-2 pointer-events-none">
                          <span className="text-xs text-muted-foreground">{keys.filter(k => form.permissions.includes(k)).length}/{keys.length}</span>
                          {isExpanded ? <ChevronDown className="w-3.5 h-3.5 text-muted-foreground" /> : <ChevronRight className="w-3.5 h-3.5 text-muted-foreground" />}
                        </div>
                      </div>
                      {isExpanded && (
                        <div className="grid grid-cols-2 gap-0.5 p-2 bg-card">
                          {group.permissions.map(perm => (
                            <label key={perm.key} className="flex items-center gap-2 px-3 py-2 rounded-lg hover:bg-muted/50 cursor-pointer">
                              <Checkbox checked={form.permissions.includes(perm.key)}
                                onCheckedChange={() => togglePerm(perm.key)}
                                className="data-[state=checked]:bg-primary data-[state=checked]:border-primary" />
                              <span className="text-sm text-muted-foreground select-none">{perm.label}</span>
                            </label>
                          ))}
                        </div>
                      )}
