...
export function CourseCompletionCelebration({
  ...
  // Helper function to get the correct view all courses path based on user role
  const getViewAllCoursesPath = () => {
    if (!user) return "/dashboard"

    switch (user.bwenge_role) {
      case "INSTRUCTOR":
        return "/dashboard/instructor/courses"
      case "INSTITUTION_ADMIN":
        return "/dashboard/institution-admin/courses"
      case "SYSTEM_ADMIN":
        return "/dashboard/system-admin/courses"
      default:
        return "/dashboard/courses"
    }
  }

  // Helper function to get the correct view course path based on user role
  const getViewCoursePath = () => {
    if (!courseId) return "#"
    if (!user) return `/courses/${courseId}`

    switch (user.bwenge_role) {
      case "INSTRUCTOR":
        return `/dashboard/instructor/courses/${courseId}`
      case "INSTITUTION_ADMIN":
        return `/dashboard/institution-admin/courses/${courseId}`
      case "SYSTEM_ADMIN":
        ...
        >
          <Card className="relative overflow-hidden border border-gray-300 bg-white shadow-xl">
            ...
            <CardContent className="p-4 text-center space-y-6">
              ...
              >
                <Button 
                  onClick={() => router.push(getViewAllCoursesPath())} 
                  size="lg"
                ...
                {courseId && (
                  <Button 
                    onClick={() => router.push(getViewCoursePath())} 
                    variant="outline" 
                    size="lg"
