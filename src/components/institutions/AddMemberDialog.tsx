...
import { addMemberToInstitution, fetchInstitutionById, fetchInstitutionLimits } from "@/lib/features/institutions/institutionSlice";
import {
  Dialog,
  DialogContent,
  DialogDescription,
...
import { Loader2, CheckCircle2, XCircle, AlertCircle, Mail, User, Phone as PhoneIcon, Users, UserCog } from "lucide-react";
...
export default function AddMemberDialog({
  ...
  const { institutionLimits, isLoadingLimits } = useAppSelector(
  ...
  // Check if adding with current role is allowed based on limits
  const isRoleAllowed = (selectedRole: InstitutionMemberRole): boolean => {
    if (!institutionLimits) return true;
    
    if (selectedRole === InstitutionMemberRole.INSTRUCTOR) {
      return institutionLimits.can_add_instructor;
    }
    return institutionLimits.can_add_member;
  };

  // Get limit error message for current role
  const getLimitErrorMessage = (): string | null => {
    if (!institutionLimits) return null;
    
    if (role === InstitutionMemberRole.INSTRUCTOR && !institutionLimits.can_add_instructor) {
      return `Cannot add instructor. Maximum limit of ${institutionLimits.max_instructors} instructor${institutionLimits.max_instructors !== 1 ? 's' : ''} reached.`;
    }
    if (!institutionLimits.can_add_member) {
      return `Cannot add member. Maximum limit of ${institutionLimits.max_members} member${institutionLimits.max_members !== 1 ? 's' : ''} reached.`;
    }
    return null;
  };
  ...
  const handleSubmit = async () => {
    ...
    const limitError = getLimitErrorMessage();
    if (limitError) {
    ...
    try {
      ...
      if (error?.data?.limit_reached) {
        const limitType = error.data.limit_type === "instructors" ? "instructors" : "members";
        toast.error(error.message || `Maximum ${limitType} limit reached.`);
      } else {
          ...
          <div className="space-y-3">
            ...
            <div className="relative">
              ...
              <Input
                ...
                disabled={isSubmitting || (institutionLimits ? !institutionLimits.can_add_member : false)}
              ...
              <div className="space-y-2">
                ...
                >
                  ...
                  <SelectContent className="bg-white p-5">
                    ...
                    <SelectItem 
                      value={InstitutionMemberRole.INSTRUCTOR}
                      disabled={!!(institutionLimits && !institutionLimits.can_add_instructor)}
                    >
                      <div className="flex flex-col items-start">
        ...
        <DialogFooter className="gap-2">
          ...
          <Button
            ...
            disabled={
              ...
              isSubmitting ||
              isCheckingEmail ||
              (institutionLimits && !isRoleAllowed(role)) ||
              (!emailCheckResult.exists &&
                (!firstName.trim() || !lastName.trim() || !username.trim()))
            }
            className="min-w-[140px]"
            title={getLimitErrorMessage() || ""}
          >
            {isSubmitting ? (
              <>
