...
// ==================== NEW: LESSON MEDIA TYPES ====================

/**
 * A single uploaded course material (PDF, DOCX, ZIP, etc.)
 * attached at the lesson level — distinct from lesson.resources
 * which are links added manually.
 */
export interface LessonMaterial {
  id?: string
  title: string
  url: string          // Cloudinary secure_url after upload
  public_id?: string   // Cloudinary public_id for deletion
  type: string         // MIME type or extension category
  size_bytes?: number  // original file size
  original_name?: string
}

/**
 * Runtime representation of a file the instructor has selected locally
 * but not yet uploaded. Used only in frontend state — never persisted.
 */
export interface PendingLessonFile {
  field: "thumbnail" | "material"  // which slot this file fills
  file: File
  previewUrl?: string               // blob URL for preview
  materialIndex?: number            // for materials array
}
...
// ==================== LESSON ====================
// NEW FIELDS:
//   thumbnail_url      — already existed, now explicitly uploadable as a File
//   video_url          — already existed, now explicitly uploadable as a File
//   lesson_materials   — NEW: array of downloadable course materials (PDFs, ZIPs, etc.)
...
export interface Lesson {
  ...
  /** Cloudinary URL of the lesson video. May temporarily be a blob: URL when a local file is chosen. */
  video_url?: string

  /** Cloudinary URL of the lesson thumbnail image. May temporarily be a blob: URL when a local file is chosen. */
  thumbnail_url?: string
  ...
  /** Manually added links / references (title + url) */
  resources?: Array<{
    title: string
    url: string
    type: string
  ...
  /**
   * NEW — Downloadable course materials uploaded as files (PDFs, DOCX, ZIP, etc.)
   * Stored as JSONB on the backend; populated with Cloudinary URLs after upload.
   */
...
export interface LessonRequest {
  ...
  /** NEW — persisted course materials */
  lesson_materials?: LessonMaterial[]
...
export function normalizeCourseFromBackend(backendCourse: any): Course {
  return {
    ...
    rating:
      typeof backendCourse.average_rating === "string"
    ...
    price:
      typeof backendCourse.price === "string"
...
export function normalizeUserFromBackend(backendUser: any): User {
  return {
    ...
    spoc_access_codes_used: Array.isArray(backendUser.spoc_access_codes_used)
      ? backendUser.spoc_access_codes_used
      : [],
