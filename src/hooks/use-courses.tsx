  ...
  const { token,user } = useAuth()
    ...
    const getCacheKey = () => {
      ...
      switch (type) {
        ...
        case "all":
          return `${baseUrl}/courses/instructor/${user.id}`
        case "org":
...
// Instructor courses fetcher - FIXED to use correct endpoint
const useInstructorCourses = (params = {}) => {
  ...
  const queryString = queryParams.toString()
  // FIX: Use the correct endpoint for instructor courses
  // Changed from /courses/instructor/my-courses to /courses/instructor/${user?.id}
  // But we need to get the user ID from the auth context
  const userId = user?.id
  
  // Don't fetch if no user ID
  const url = token && userId 
    ? `${process.env.NEXT_PUBLIC_API_URL}/courses/instructor/${userId}${queryString ? `?${queryString}` : ''}`
    : null
